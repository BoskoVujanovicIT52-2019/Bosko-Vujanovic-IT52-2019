...
public class PnlDrawing extends JPanel implements MouseListener{
	...
	public void modify() {		
		if (shapes.isEmpty())
			JOptionPane.showMessageDialog(null, "There are no shapes to Modify!");
		else if (selected == null)
			JOptionPane.showMessageDialog(null, "No shape selected!");
		else
		{
			...
			} else if (shapes.get(i).isSelected() && shapes.get(i) instanceof Rectangle)
			{
				...
				dlg.getTX().setText(String.valueOf(temp.getX()));
				dlg.getTY().setText(String.valueOf(temp.getY()));
				dlg.getBtnColor().setBackground(temp.getColor());
				dlg.setTitle("Modify point");
				dlg.setVisible(true);
				if (dlg.isOk()) {
						Point p3 = new Point(Integer.parseInt(dlg.getTX().getText().toString()), Integer.parseInt(dlg.getTY().getText().toString()), false, dlg.getBtnColor().getBackground());
						shapes.add(p3);
						shapes.remove(shapes.get(i));		
						selected = null;
				}
